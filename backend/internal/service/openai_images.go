...
// validateOpenAINativeImagesModel 校验真正要发往 ChatGPT 的上游模型：OAuth / setup-token
// 路径只认 OpenAI 自家图片模型，不走 validateOpenAIImagesModel 里对 Adobe 对外名
// （gemini-*-image 等）的放行，否则 Gemini 图片模型会被原样转发给 OpenAI。
func validateOpenAINativeImagesModel(model string) error {
	model = strings.TrimSpace(model)
	if isOpenAIImageGenerationModel(model) {
		return nil
	}
