...
func TestResolveGeminiThinkingVariant(t *testing.T) {
	...
	tests := []struct {
		...
		// 空 credentials 映射 → 走 DefaultAntigravityModelMapping；本 fork 把 3.8 flash 全族
		// 收敛到 -tiered，裸名 "gemini-3.8-flash" 在默认目录里本身就有一条非自身的显式映射
		// （→ -tiered，见 domain.TestDefaultAntigravityModelMapping_Gemini38FlashModels）。
		// resolveGeminiThinkingVariant 的"裸名已有真正映射就尊重现有配置"规则（分支 1）
		// 因此不介入推导，交回常规 getMappedModel 流程（它会走到同一个 -tiered 结果）。
		{"empty mapping falls back to default catalog", map[string]string{}, "gemini-3.8-flash", budget("-1"), "", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
